...
export class ClientsService {
    ...
    async create(data: { name: string; company_id: string }, userRole: string, userCompanyId: string) {
    ...
    async findAll(userRole: string, userCompanyId: string) {
        if (userRole === 'COMPANY_ADMIN') {
            return this.prisma.client.findMany({ where: { company_id: userCompanyId } });
        }
    ...
    async findOne(id: string, userRole: string, userCompanyId: string, userClientId: string) {
        ...
        if (userRole === 'COMPANY_ADMIN' && client.company_id !== userCompanyId) {
            throw new ForbiddenException('You can only access clients from your company');
        }
    ...
    async update(id: string, data: { name?: string }, userRole: string, userCompanyId: string, userClientId: string) {
        const client = await this.prisma.client.findUnique({ where: { id } });
        if (!client) throw new NotFoundException('Client not found');

        if (userRole === 'COMPANY_ADMIN' && client.company_id !== userCompanyId) {
            throw new ForbiddenException('You can only update clients from your company');
        }
        if (userRole === 'CLIENT_ADMIN' && client.id !== userClientId) {
