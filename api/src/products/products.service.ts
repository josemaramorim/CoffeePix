...
export class ProductsService {
  ...
  async create(
    data: { name: string; price: number; company_id: string },
    userRole: string,
  ...
  async findOne(
    ...
    if (userRole === 'COMPANY_ADMIN' && product.company_id !== userCompanyId) {
      throw new ForbiddenException(
        'You can only access products from your company',
      );
    }
    if (userRole === 'CLIENT_ADMIN') {
      const client = await this.prisma.client.findUnique({
      ...
      if (!client || product.company_id !== client.company_id) {
        throw new ForbiddenException(
          'You can only access products from your company catalog',
        );
      }
    }
  ...
  async update(
    ...
    if (userRole === 'COMPANY_ADMIN' && product.company_id !== userCompanyId) {
      throw new ForbiddenException(
        'You can only update products from your company',
      );
    }
